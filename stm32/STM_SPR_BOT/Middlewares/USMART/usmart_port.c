...
char *usmart_get_input_string(void)
{
    ...
    if (USART1_RX_DATA_Struct.State & 0x8000)        /* 串口接收完成？ */
    {
        len = USART1_RX_DATA_Struct.State & 0x3fff;  /* 得到此次接收到的数据长度 */
        USART1_RX_DATA_Struct.pDatas[len] = '\0';     /* 在末尾加入结束符. */
        pbuf = (char*)USART1_RX_DATA_Struct.pDatas;
        USART1_RX_DATA_Struct.State = 0;             /* 开启下一次接收 */
    }
